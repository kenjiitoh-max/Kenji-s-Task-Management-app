...
const colors = ['#F97362', '#3B82F6', '#8B5CF6', '#10B981', '#F59E0B', '#EC4899'];
...
export function CategoryFormModal({ visible, dark, onClose, onSave }: { visible: boolean; dark: boolean; onClose: () => void; onSave: (name: string, color: string) => void }) {
  ...
  return (
    <ModalShell dark={dark} onClose={onClose} title="カテゴリーを追加" visible={visible}>
      ...
      <View style={styles.colors}>{colors.map((item) => <Pressable key={item} onPress={() => setColor(item)} style={[styles.color, { backgroundColor: item }, color === item && styles.selected]} />)}</View>
